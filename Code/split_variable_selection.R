...
compute_residual <- function(
  ...
  ###                    zip, zinb, hp, hnb)
  ### @param true_counts: Ground truth.
...
cov_name_types <- get_cat_con(school_absences)
...
predictors <- names(school_absences)[-1]
...
for (cov_name_type in cov_name_types) {
  ...
  cov_data <- school_absences[cov_name][,]
  cov_data_grp <- NA
  if (cov_type == "con") {
